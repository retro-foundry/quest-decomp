...
- runtime `$1E6C-$1F0E`, loaded `$366C-$370E`,
  `initialise_indexed_xor_graphics_for_room`;
...
- runtime `$343A-$3452`, loaded `$4C3A-$4C52`,
  `reverse_indexed_xor_graphic_delta_at_limits`; and
- runtime `$3453-$3487`, loaded `$4C53-$4C87`,
  `draw_indexed_xor_graphic`; and
- runtime `$3488-$34BA`, loaded `$4C88-$4CBA`,
  `advance_indexed_xor_graphic_state_and_pointer`; and
...
The indexed delta-limit helper compares Y-indexed selector state with the
...
The indexed draw wrapper maps Y-indexed `$122A/$64` state to all four observed
...
The adjacent updater adds its Y-indexed `$01/$FF` delta to selector state and
